...
@Component
@Slf4j
public class BlockFollowTestComponent {
    @Autowired
    ...
    public List<BlocKFollowStaticBuyDTO> blockFollowBuyStock(List<String> plankStocks){
        ...
        List<BlocKFollowStaticBuyDTO> allStocks = Lists.newArrayList();
        for (HistoryBlockInfo historyBlockInfo:historyBlockInfos){
            //板块内股票
            List<String> stocks = getBlockStocks(historyBlockInfo.getBlockCode(), DateUtil.format(preTradeDate, DateUtil.yyyyMMdd));
            //判断板块内涨停数量
            List<String> blockPlankStocks = blockPlanks(stocks, plankStocks);
            if(blockPlankStocks.size()<3){
                continue;
            }
            log.info("板块跟随符合3块板的板块blockCode:{} plankStock:{}",historyBlockInfo.getBlockCode(),JSONObject.toJSONString(blockPlankStocks));
            //获取板块内小市值购买部分
    ...
    public List<String> blockPlanks(List<String> stocks,List<String> planks){
        List<String> blockStockPlanks = Lists.newArrayList();
        if(CollectionUtils.isEmpty(stocks)){
            return blockStockPlanks;
        }
        for (String stock:stocks){
            if(planks.contains(stock)){
                blockStockPlanks.add(stock);
            }
        }
        return blockStockPlanks;
    }
