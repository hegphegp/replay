...
@Component
@Slf4j
public class BlockFollowZZ1000Current3PlankComponent {
    @Autowired
    ...
    public static final ExecutorService THREAD_POOL_QUOTE_4PLANK = ThreadPoolUtils.create(4, 32, 512, "QuoteThreadPool");
    ...
    public void blockFollowStaticInfo(){
        ...
        for (String tradeDate:pairsMap.keySet()) {
            ...
            if(tradeDate.equals("20220518")){
                System.out.println(11111111);
            }else{
                continue;
            }

            THREAD_POOL_QUOTE_4PLANK.execute(() ->{
                List<PlankTimePairDTO> plankTimePairDTOS = pairsMap.get(tradeDate);
                BlocKFollowStaticTotalDTO blocKFollowStaticTotalDTO = blockBuys(3,blockInfos, plankTimePairDTOS, circulateInfoMap, tradeDate);
        ...
        for (CirculateInfo circulateInfo:circulateInfos){
           ...
           /* if(m>=500){
            ...
            for (StockKbar stockKbar:stockKbars){
                ...
                if(date.before(DateUtil.parseDate("20211215", DateUtil.yyyyMMdd))){
                    continue;
                }
                if(date.after(DateUtil.parseDate("20220205", DateUtil.yyyyMMdd))){
                    continue;
                }
    ...
    public BlocKFollowStaticTotalDTO blockBuys(int count,List<HistoryBlockInfo> blockInfos,List<PlankTimePairDTO> pairs,Map<String, CirculateInfo> circulateInfoMap,String tradeDate){
        ...
        for (HistoryBlockInfo blockInfo:blockInfos){
            ...
            List<String> stocks = getBlockStocks(blockInfo.getBlockCode(), DateUtil.format(preTradeDate,DateUtil.yyyyMMdd));
            ...
            Long threePlankTime = judgeThreePlanks(blockPairs,count);
            Long threeFirstPlankTime = judgeThreeFirstPlanks(blockPairs,count);
            if(plankTens.size()>=count){
            ...
            if(plankTens.size()>=count&&flagThreePlank&&flagFristThreePlank){
                allBuyCount++;
            }
            if(plankTens.size()>=count||flagThreePlank||flagFristThreePlank){
                oneBuyCount++;
        ...
        for (BlocKFollowStaticBuyDTO dto:dtos){
            ...
            for (StockKbar stockKbar:stockKbars){
                if(flag){
                    ...
                    if(!endUpper){
                        if(stockKbar.getAdjFactor()!=null&&stockKbar.getAdjFactor().equals(buyKbar.getAdjFactor())){
                            BigDecimal profitEnd = PriceUtil.getPricePercentRate(stockKbar.getClosePrice().subtract(dto.getBuyPrice()), dto.getBuyPrice());
            ...
            for (StockKbar stockKbar:stockKbars){
                ...
                if(days==1){
                    ...
                    if(stockKbar.getAdjFactor()!=null&&stockKbar.getAdjFactor().equals(buyKbar.getAdjFactor())){
                        BigDecimal profitTen = PriceUtil.getPricePercentRate(tenAvgPrice.subtract(dto.getBuyPrice()), dto.getBuyPrice());
    ...
    public Long judgeThreePlanks(List<PlankTimePairDTO> pairs,int count){
    ...
    public Long judgeThreeFirstPlanks(List<PlankTimePairDTO> pairs,int count){
        List<PlankTimePairDTO> pairsCopy = Lists.newArrayList();
        pairsCopy.addAll(pairs);
        for (PlankTimePairDTO pair:pairs){
            if(pair.getPlanks()!=1){
        ...
        for (BlocKFollowStaticBuyDTO dto:buyDTOS){
            ...
            if(buyPrice!=null){
                ...
                if(stockKbar.getAdjFactor()!=null && stockKbar.getAdjFactor().equals(preStockKbar.getAdjFactor())){
                    BigDecimal rate = PriceUtil.getPricePercentRate(buyPrice.subtract(preStockKbar.getClosePrice()), preStockKbar.getClosePrice());
                    dto.setRate(rate);
                }else {
            ...
            if (buyPrice != null && sellPrice != null) {
                if(stockKbar.getAdjFactor()!=null && stockKbar.getAdjFactor().equals(nextStockKbar.getAdjFactor())){
                    BigDecimal profit = PriceUtil.getPricePercentRate(sellPrice.subtract(buyPrice), buyPrice);
    ...
    public Map<String,BigDecimal> getZz1000Rate(){
        List<StockKbar> stockKBarsDelete30Days = getStockKBarsDelete30Days("000852");
        return null;
    }
